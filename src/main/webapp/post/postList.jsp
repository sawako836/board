		...
		<div class="row">
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<form id="getPost" action="${cp }/postDetail" method="get">
					<input id="pnum" type="hidden" name="pnum"> <input
						id="inum" type="hidden" name="inum">
				</form>
				...
				<c:choose>
					<c:when test="${post.deletion == 'n'}">
						<tr class="deleteTr">
							...
							<td><c:forEach begin="0" end="${post.layer}" var="i">
									<c:if test="${i > 0}">
												&nbsp;&nbsp;&nbsp;
											</c:if>
								</c:forEach> <c:if test="${post.layer > 0}">
...
<%-- 					<c:forEach items="${postList }" var="post">
			...
			<button id="insertPost" type="button"
				class="btn btn-default pull-right">새글 등록</button>
			...
			<div class="text-center">
				<ul class="pagination">
					...
					<c:choose>
						...
						<c:otherwise>
							<li><a
								href="${cp}/postPagingList?page=1&pagesize=10&inum=${inum}"
					...
					<c:choose>
						...
						<c:otherwise>
							<li><a
								href="${cp}/postPagingList?page=${page-1}&pagesize=10&inum=${inum}"
					...
					<c:forEach begin="1" end="${paginationSize}" var="selectpage">
						<c:choose>
							...
							<c:otherwise>
								<li><a
									href="${cp}/postPagingList?page=${selectpage}&pagesize=10&inum=${inum}">${selectpage}</a></li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<!-- 페이지네이션 방향 : > -->
					...
					<c:choose>
						...
						<c:otherwise>
							<li><a
								href="${cp}/postPagingList?page=${paginationSize}&pagesize=10&inum=${inum}"
